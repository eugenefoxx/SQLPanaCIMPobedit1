...
func main() {
	...
	//goRfcStorage := gosaprfc.NewGoRFCRepository(&logger)
	...
	resGetJobID1, err := jobIdStorage.GetLastJobIdValue1()
	if err != nil {
		logger.Errorf(err.Error())
	}
	if resGetJobID1 != "" {
		...
		productIdSlice, err := panacimStorage.GetProductId(resGetJobID1)
		if err != nil {
			logger.Errorf(err.Error())
		}
		...
		//var sumPCBFromU03 string
		//wg := sync.WaitGroup{}
		//wg.Add(1)
		//go func() {
		sumPCBFromU03 := panacimStorage.GetSumPCBFromU03V2(string(unixSlice[0].StartUnixTimeWO), string(unixSlice[0].EndUnixTimeWO), npm)
		//go panacimStorage.GetSumPCBFromU03V2(string(unixSlice[0].StartUnixTimeWO), string(unixSlice[0].EndUnixTimeWO), npm)
		//	wg.Done()
		//}()
		//wg.Wait()
		...
		if routeid == "1009" {
			...
			if err != nil {
				//	println(err.Error())
				logger.Errorf("проверить параметры CLI readDGSP1forKATE %v", err.Error())
				return
			}
		}
		/////////////// Блок логики с БД PanaCIM
		// получаем потребленные компоненты их кол-ва по job_id
		componentsSlice, err := panacimStorage.GetPanacimDataComponentsByJobId(resGetJobID1)
		if err != nil {
			logger.Errorf(err.Error())
		}
		...
		componentIdSlice, err := panacimStorage.GetPanacimDataComponentsByJobIdAllParamReelid(resGetJobID1)
		if err != nil {
			logger.Errorf(err.Error())
		}
		...
		// получить wo_component
		woComponentSlice, err := panacimStorage.GetPanaCIMDBWOComponent(resGetJobID1)
		if err != nil {
			logger.Errorf(err.Error())
		}
		if err := panacimStorage.WritePanaCIMDBWOComponentToFile(woComponentSlice); err != nil {
			logger.Errorf(err.Error())
		}
		// получить scrap_id
		scrapIDSlice, err := panacimStorage.GetPanaCIMDBScrapID(resGetJobID1)
		if err != nil {
			logger.Errorf(err.Error())
		}
		if err := panacimStorage.WritePanaCIMDBScrapIDToFile(scrapIDSlice); err != nil {
			logger.Errorf(err.Error())
		}
		// получить scrap part_number, sum, lot
		scrapSclice, err := panacimStorage.GetPanaCIMDBScrap(resGetJobID1)
		if err != nil {
			logger.Errorf(err.Error())
		}
		if err := panacimStorage.WritePanaCIMDBScrapToFile(scrapSclice); err != nil {
			logger.Errorf(err.Error())
		}
		/////////////////

		// получить work order name
		wo_nameSlice, err := panacimStorage.GetWOName(resGetJobID1)
		if err != nil {
			logger.Errorf(err.Error())
		}
		//var woOrder string
		...
		*/
		...
		npmRecipe, err := filereader.Readfileseeker(recipe)
		if err != nil {
			logger.Errorf(err.Error())
		}
		...
		substituteCsvFormattedRemove := substituteCsvFormatted
		if utils.FileExists(substituteCsvFormattedRemove) {
			os.Remove(substituteCsvFormattedRemove)
		}
		...
		reportParts, err := filereader.Readfileseeker(substituteCsvFormatted)
		if err != nil {
			logger.Errorf(err.Error())
		}
		fmt.Println("test read file")
		//panacimdata := readfileseeker("/home/eugenearch/Code/github.com/eugenefoxx/SQLPanacimP1/csvfolder/panacim.csv")
		panacimdata, err := filereader.Readfileseeker(panacimCsv)
		if err != nil {
			logger.Errorf(err.Error())
		}
		...
		summaryReportComponents(reportSummary, resGetJobID1, sumPCB)
		summaryReportComponentsToFile(reportSummary, numberOrder, sumPCB)
		//var i int
		/*
		...
		*/
		...
		logger.Println("resGetJobID1 нет новых данных")
	}
	resGetJobID2, err := jobIdStorage.GetLastJobIdValue2()
	if err != nil {
		logger.Errorf(err.Error())
	}
	if resGetJobID2 != "" {
		logger.Infof(("resGetJobID2 - %v"), resGetJobID2)
	} else {
		logger.Println("resGetJobID2 нет новых данных")
	}
	resGetJobID3, err := jobIdStorage.GetLastJobIdValue3()
	if err != nil {
		logger.Errorf(err.Error())
	}
	if resGetJobID3 != "" {
		logger.Infof(("resGetJobID3 - %v"), resGetJobID3)
	} else {
		logger.Println("resGetJobID3 нет новых данных")
	}
