			...
			if id == pana_id {
				if i[4] == "7814" {
					if i[5] == spp {
						...
						r.logger.Errorf("СПП, при наличии не соотвествует %v", i[5])
						response := false
						return response, nil
					}
							...
							if pana_pn == pn {
								...
								if result >= 0 {
									...
									if pana_curren_qty <= result {
										...
										r.logger.Errorf("кол-во после освобождения ео не соотвествует требованию проверки: ео %v, кол-во остатка в panacim %v, вычисляемы остаток %v", pana_id, j.CurrentQuantity, result)
										response := false
										return response, nil
									}
								} else {
									r.logger.Errorf("кол-во после освобождения ео не соотвествует требованию проверки, меньше ноля: ео %v, кол-во остатка в panacim %v, вычисляемы остаток %v", pana_id, j.CurrentQuantity, result)
									response := false
									return response, nil
								}
							} else {
